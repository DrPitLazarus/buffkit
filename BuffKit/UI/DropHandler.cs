...
/// <summary>
/// Assign this to a layout group to make its children draggable and droppable. Adds Dragger component to each child. 
/// The order of the children will be preserved in the OnDropped event.
/// </summary>
public class DropHandler : MonoBehaviour
{
    public event Action<SortedList<int, Transform>, SortedList<int, Transform>> OnDropped;

    private SortedList<int, Transform> _startingOrder = [];
    ...
    private void Dropped()
    {
        ...
        {
            var c = transform.GetChild(i);
            if (c.GetComponent<Dragger>() is not null && c.gameObject.activeInHierarchy)
            {
...
public class Dragger : MonoBehaviour, IDragHandler, IBeginDragHandler, IEndDragHandler, IPointerUpHandler
{
    ...
    public void OnDrag(PointerEventData eventData)
    {
        ...
        for (int i = 0; i < transform.parent.childCount; i++)
        {
            ...
            if (transform.parent.GetChild(i).GetComponent<RectTransform>() is not RectTransform t) continue;
