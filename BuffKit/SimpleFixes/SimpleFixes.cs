...
public class SimpleFixes
{
    ...
    /// <summary>
    /// Fix UICrewOrdersDisplay not hiding when scoreboard is shown.
    /// </summary>
    [HarmonyPatch(typeof(UIManager.UIGamePlayState), nameof(UIManager.UIGamePlayState.HandleOverlayToggles))]
    [HarmonyPostfix]
    private static void ToggleUICrewOrdersDisplayOnScoreboard()
    {
        if (IcarusInput.ScoreLogHeld)
            UICrewOrdersDisplay.Deactivate();
        else
            UICrewOrdersDisplay.Activate();
    }
}
